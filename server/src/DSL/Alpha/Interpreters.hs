...
logI (GetModel _)       = Log.log $ printf "Get model"
...
logI (SetMaxLife w l _) = Log.log $ printf "Set max life %s %s" (show w) (show l)
...
diffI (SetMaxLife w l _) diff = modPmodelDiff (\pm -> pm { pmodeldiff_maxLife = Just l }) w diff
